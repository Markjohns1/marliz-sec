            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="flex justify-end items-center">
                    ...
                    >
                        ...
                        {isUploading ? 'Optimizing...' : 'Upload & Secure'}
                    </button>
                </div>
            </div>
                        ...
                        <div key={item.id} className="group relative bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden transition-all hover:border-emerald-500/50 hover:shadow-2xl">
                            <div className="aspect-video relative bg-slate-950 overflow-hidden flex items-center justify-center">
                                ...
                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                                    ...
                                    <button onClick={() => copyToClipboard(item.url)} className="p-2.5 bg-white/10 hover:bg-white/20 text-white rounded-lg backdrop-blur-md" title="Copy URL">
                                ...
                                <div className="relative group/alt bg-slate-950 border border-slate-800 rounded-xl p-3 transition-all hover:border-emerald-500/30">
                                    ...
                                    {editingAltId === item.id ? (
                                        <input
                                            ...
                                            className="w-full bg-transparent text-xs text-white focus:outline-none border-b border-emerald-500/50 pb-1 mt-1"
                                            autoFocus
                                            placeholder="Write high-impact SEO alt text..."
                                        />
                                    ) : (
                                        <p className={`text-xs ${item.alt_text ? 'text-slate-300' : 'text-slate-600 italic'} truncate mt-1`}>
                                            {item.alt_text || 'No Alt Text - Low SEO Score'}
                                        </p>
                                    )}
                                </div>
